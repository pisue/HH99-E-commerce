...
### 1. 🧪 테스트 보완
- [ ] **Saga 패턴 통합 테스트:** `OrderFacade`의 보상 트랜잭션(재고 및 잔액 복구)이 예외 상황에서 정확히 동작하는지 검증.
- [ ] **동시성 테스트:** 대량의 주문 요청 시 재고와 잔액의 데이터 정합성 검증.

### 2. 🛡️ 안정성 강화
- [ ] **카프카 멱등성(Idempotency) 처리:** 컨슈머에서 중복된 메시지 수신 시 처리 로직 구현 (Order ID 기반 중복 체크).
- [ ] **API 예외 처리 표준화:** `GlobalExceptionHandler` 고도화 및 에러 응답 형식 통일.

### 3. ⚡ 성능 최적화
- [ ] **Redis 기반 재고 관리:** 현재 DB 비관적 락 방식을 Redis(Lua Script)로 이관하여 처리 속도 개선.
- [ ] **캐시 전략 고도화:** 단순 조회를 넘어선 다양한 데이터 캐싱 전략 수립.

### 4. 📊 모니터링
- [ ] **분산 추적(Distributed Tracing) 도입:** Zipkin이나 Jaeger 등을 활용하여 마이크로서비스 간 트랜잭션 흐름 시각화.
